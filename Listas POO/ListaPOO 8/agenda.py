...
class Agenda:
    def __init__(self, id, data, idCliente, idServico, confirmado):
        self.__id = id
        self.__data = data
    ...
    def __str__(self):
        return f"{self.__id} - {self.__data.strftime('%d/%m/%Y %H:%M')} - {self.__idCliente} - {self.__idServico} - {self.__confirmado}"
    ...
    def abrir(cls):
        try:
            ...
            with open("agendas.json", mode="r") as f:
                s = json.load(f)
                for agenda in s:
                    a = Agenda(agenda["__id"], datetime.datetime.strptime(agenda["__data"], '%d/%m/%Y %H:%M'), agenda["__idCliente"], agenda["__idServico"], agenda["__confirmado"])
