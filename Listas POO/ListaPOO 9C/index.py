...
class IndexUI:
    ...
    def sidebar():
        op = st.sidebar.selectbox("Menu", [
            "Manter Clientes", "Manter Serviços", "Manter Agenda",
            "Abrir Agenda do Dia", "Confirmar Agendamento", "Abrir Conta"
        ])
        ...
        if op == "Abrir Agenda do Dia":
            st.session_state["page"] = "abriragendaUI"
        if op == "Abrir Conta": st.session_state["page"] = "abrircontaUI"

    def main():
        IndexUI.sidebar()
        if "page" not in st.session_state:
            st.session_state["page"] = "clienteUI"
